...
export const historySlice = createSlice({
  ...
  reducers: {
    ...
    addItemToStatistic: (state, action: PayloadAction<number>) => {
      ...
      if (state.statisticItems && state.statisticItems[value]) {
      ...
      localStorage.setItem("statisticCount", JSON.stringify(state.statisticItems));
    },
    clearHistory: (state) => {
      state.items = [];
...
export const { clearHistory, addItemToHistory, addItemToStatistic, clearStatistics } = historySlice.actions;
