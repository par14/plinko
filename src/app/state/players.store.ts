...
export const PlayersStore = signalStore(
  ...
  withMethods((store, db = inject(PLINKO_STORE)) => {
    const adjustPlayerBalance = (id: string, delta: number): void => {
      patchState(store, (s) => ({
    ...
    return {
      ...
      /** Adds (or subtracts) from one player's balance and persists. */
      adjustPlayerBalance,
      /** Adds (or subtracts) from the currently active player's balance. */
      adjustBalance(delta: number): void {
        const id = store.activePlayerId();
        if (id) adjustPlayerBalance(id, delta);
      },
