...
import { selectActiveBalls, selectLinesCount, selectRiskMode } from "@store/config/config.selectors";
import type { HistoryItem } from "@store/history/history.interface";
import { addGameRunning, removeGameRunning, setLastWin } from "@store/config/config.slice";
import { addItemToHistory, addItemToStatistic } from "@store/history/history.slice";
import { getHolesByLine, getHoleSound } from "@pages/Plinko/components/Game/utils/getHoles";
import { getRateForLine } from "@pages/Plinko/components/Game/utils/getRateForLine";
import ballAudio from "@assets/sounds/ball.wav";
import withPin from "@assets/sounds/withPin.ogg";
...
export function Game() {
  ...
  for (let l = 0; l < linesCount; l++) {
    ...
    for (let i = 0; i < linePins; i++) {
      ...
      const pinY = ACTIVE_AREA_HEIGHT / linesCount + l * configSizes.pinGap + DISTANCE_FROM_TOP_FLOOR;
  ...
  const play = useCallback((ballValue: number) => {
      addInGameBall();
      ...
      const ball = Bodies.circle(MAX_WORLD_WIDTH / 2 + random(-3, 3), DISTANCE_FROM_TOP_FLOOR, configSizes.ballSize, {
  ...
  const leftWall = Bodies.rectangle(
    linesCount < 12 ? MAX_WORLD_WIDTH / 2 - configSizes.pinSize * 2 : MAX_WORLD_WIDTH / 2,
    0,
    MAX_WORLD_WIDTH * 2,
    20,
    {
  ...
  let firstHolePositionX: number =
    MAX_WORLD_WIDTH / 2 - (configSizes.pinGap / 2) * linesCount - configSizes.pinGap;
  ...
  function onTouchHole(ball: Body, multiplier: Body) {
    ...
    const holeSong = new Audio(getHoleSound(linesCount, riskMode, holeValueFromLabel));
    playSong(holeSong);
